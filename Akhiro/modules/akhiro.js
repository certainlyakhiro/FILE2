...
module.exports = {
  ...
  onRun: async ({ api, event, args }) => {
    ...
    if (!query) {
      api.sendMessage(fonts.sans("ℹ️ | Please provide a query.", event.threadID, event.messageID));
      api.setMessageReaction("ℹ️", event.messageID, (err) => {}, true);
      return;
    }
    api.setMessageReaction("⏳", event.messageID, (err) => {}, true);
    try {
      const response = await axios.get(`https://lianeapi.onrender.com/@unregistered/api/🙋‍♂️|akhiroai?query=${encodeURIComponent(query)}`);
